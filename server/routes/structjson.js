// convert between JSON and goog.protobuf.Struct proto

'use strict'

function jsonToStructProto(json) {
    const fields = {}
    for (let k in json) {fields[k] = jsonValueToProto(json[k])}
    return {fields}
}
...
function jsonValueToProto(value) {
    ...
    else {console.warn('Unsupported value type ', typeof value)}
    return valueProto
}
...
function structProtoToJson(proto) {
    ...
    const json = {}
    for (const k in proto.fields) {json[k] = valueProtoToJson(proto.fields[k])}
    return json
}

function valueProtoToJson(proto) {
    if (!proto || !proto.kind) {return null}

    if (JSON_SIMPLE_VALUE_KINDS.has(proto.kind)) {return proto[proto.kind]} 
    else if (proto.kind === 'nullValue') {return null} 
...
module.exports = {jsonToStructProto, structProtoToJson}
